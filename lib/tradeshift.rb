...
class Tradeshift
    ...
    def documents_list_for_company(company_account_id)
        ...
        response = HTTParty.get('https://api.tradeshift.com/tradeshift/rest/external/documents/')
        ...
        (numPages - 1).times do |i|
    	    ...
    	    @esponse = HTTParty.get("https://api.tradeshift.com/tradeshift/rest/external/documents/?page=#{i}&limit=#{itemsperpage}")
	        parsed = JSON.parse(response)
	        results.push(parsed["Document"])
        end
        return results

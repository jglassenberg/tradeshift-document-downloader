...
class Tag
  ...
  openTag: ->
    ...
    @attributes['id'] = @attributes['name'] if @attributes['name']? and !@attributes['id']
...
@HtmlHelpers =
  digitToWord: (digit, opts={}) ->
    throw "Can only translate single digits" unless /[0-9]/.match digit
    word = switch digit
      when 1 then 'one'
      when 2 then 'two'
      when 3 then 'three'
    ...
    if opts.capitalize then (word.charAt(0).toUpperCase() + word.slice(1)) else word

  imageTag: (src, options={}) ->
    options['src'] ||= src
    Tag.make('img', options, null).toString()
  ...
  textArea: (name, value = "", options = {}) ->
    options['name'] = name
    Tag.make('textarea', options, value).toString()
